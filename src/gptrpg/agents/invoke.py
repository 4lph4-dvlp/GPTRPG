...
CLASSIFIER_TIMEOUT_S = 15.0
"""action_classifier 호출 타임아웃 (D-27을 2026-08-17에 5초에서 올린 값).

**왜 올렸나.** IN-01 리뷰가 예고한 위험이 실제 플레이에서 그대로 터졌다 —
플레이어가 긴 문장을 치면 두 시도(D-28)가 모두 5초에 걸려 화면에는 "무브
없음"으로만 보였다. 그 자리에서 **모델을 바꾸면 되는지**를 먼저 재봤고,
답은 아니었다. 같은 프롬프트·같은 경로로 라이브 측정한 결과:
...
**5초 안에 안정적으로 들어오는 모델이 하나도 없다.** 즉 D-27이 깔고 있던
「분류기는 경량 모델이니 5초면 된다」는 전제 자체가 현실에 없다. 한국어
자유 문장을 쓸 만한 품질로 분류하려면 큰 모델이 필요하고, 큰 모델은 5초를
넘긴다 — 두 조건이 양립하지 않는다.

값을 15초로 둔 이유는 관측된 최악(9.8초)에 여유를 두면서 `GM_TIMEOUT_S`와
같은 층으로 맞추기 위해서다. 분류기만 유독 낮게 잡을 근거가 사라졌다.

**대가.** 애매한 긴 입력에서 최악 대기가 시도당 15초까지 늘어난다(보통은
2~5초). D-28의 두 번 시도는 그대로이므로 완전 실패까지는 최악 30초다."""
