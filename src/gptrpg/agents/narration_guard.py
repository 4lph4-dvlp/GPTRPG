...
_INVITATION_ADDRESS_TERMS = ("당신", "너희", "그대", "여러분")
"""오프닝이 플레이어를 직접 부를 때 쓰는 한국어 2인칭 호칭의 닫힌
목록(verify-13-06 결함3, SCENE-02 다섯째 요소 「규칙 용어 없는 열린
초대」). 규칙 용어(무브·능력치·판정)가 아니라 일반 호칭 낱말만 골랐다 —
이 목록 자체가 새 규칙 용어를 만드는 일이 되면 안 된다(SCENE-02와 같은
규율)."""


def _has_invitation_signal(text: str) -> bool:
    """오프닝 전체 텍스트에 「열린 초대」로 읽을 수 있는 신호가 있는지
    본다(SCENE-02 다섯째 요소, verify-13-06 결함3). 새 AI 호출 없이
    문자열만 본다 — `inspect_opening_completeness`의 세 번째 검사가
    이 함수를 그대로 쓴다.

    **신호 둘 중 하나면 통과다:** ① 물음표(반각 `?`/전각 `？`)가 텍스트
    어디에든 있다 ② `_INVITATION_ADDRESS_TERMS`(당신/너희/그대/여러분)
    중 하나로 플레이어를 직접 부른다.

    **왜 물음표 하나만 신호로 삼지 않았는가**(사장님이 준 예 그대로
    쓰지 않은 이유). 한국어 초대가 항상 의문문은 아니다 — "이제 당신
    차례다."는 물음표 없이도 명백한 초대이지만 순수 물음표 검사는
    이것을 놓친다. 그래서 호칭 신호를 OR로 더했다. **반대로 호칭 신호만
    쓰지 않은 이유**는 그 반대 방향의 구멍이다 — 한국어는 주어를 흔히
    생략하므로(pro-drop) "지금 무엇을 하시겠습니까?"처럼 「당신」이
    문장에 안 적혀 있어도 존댓말 종결(-시겠습니까)로 플레이어에게 묻는
    문장이 흔하다. 물음표를 독립 신호로 남겨야 이런 문장을 놓치지
    않는다. `lamplight_vigil`의 실제 초대("당신들은 지금 무엇을
    하시겠습니까?")는 두 신호 모두를 낸다 — 겹쳐도 문제없다, OR이다.

    **이 검사가 정직하게 놓치는 것.** 호칭도 물음표도 없는 명령형
    초대("행동을 정하라.", "다음을 정해라.")는 두 신호 중 어느 것도
    걸리지 않아 통과로 잘못 읽힌다. 완벽한 초대 탐지가 아니라 이번에
    실제로 관찰된 실패(초대 자체가 통째로 빠진, 분위기만 있는 오프닝)를
    잡는 최소 검사다 — `hook_terms` 검사가 이미 인정한 것과 같은 급의
    한계다(위 도크스트링 "나머지 요소는 이 검사가 못 잡는다" 참조)."""
    if "?" in text or "？" in text:
        return True
    return any(term in text for term in _INVITATION_ADDRESS_TERMS)
...
def inspect_opening_completeness(text: str, opening: OpeningDecl) -> tuple[str, ...]:
    ...
    verify-13-06 결함3이 더한 초대 검사). 위반 사유 튜플을 돌려준다 —
    **빈 튜플이 통과다.**

    **`inspect_sentence`와 다른 자리에 있는 이유.** 위 갈래들은 전부
    `sentence: str, next_sentence: str | None`을 받는 **한 문장** 판정
    함수다(생각 블록·원문 겹침·자기 반복·캐릭터 이탈·깨진 글자 다섯 다).
    반면 D-07의 다섯 요소(내가 누구인지/보이고 들리는 것/왜 중요한지/
    실마리/열린 초대)는 **오프닝 전체의 완결성**이라 문장 하나로는
    판단할 수 없다 — "실마리가 있는가"·"초대가 있는가"는 오프닝 전체를
    봐야 안다. D-08의 문언("서사가 이미 거치는 네 갈래 검사에 한 갈래를
    더한다")과 이 구현(별도 문서 단위 함수)의 차이는 13-RESEARCH.md가
    A2로 명시적으로 flag했고, 13-03 Task 1 체크포인트에서 사장님이
    확인·승인했다(SUMMARY 기록).

    **이 함수가 실제로 보는 것은 셋뿐이고, 그 이상을 주장하지 않는다:**
    ① `text`가 `strip()` 뒤 비어 있으면 위반. ② `opening.hook_terms` 중
    어느 것도 `text` 안에 없으면 위반 — 대조는 NFC 정규화 + 공백 제거 뒤
    부분 문자열 포함으로 한다. ③ `_has_invitation_signal(text)`가
    거짓이면 위반(verify-13-06 결함3 — 실마리는 있지만 「그래서 이제
    무엇을 하라」는 초대가 통째로 빠진 오프닝이 실제로 났다. 분위기만
    있고 플레이어에게 아무것도 쥐여 주지 않는 오프닝은 SCENE-02가
    요구하는 다섯째 요소를 어긴다). **"왜 중요한지" 등 나머지 요소는
    이 검사가 못 잡는다** — 사장님이 명시적으로 받아들인 한계다(등록
    시점 구조 검사가 다섯 칸이 채워졌음을 보장하고, 이 검사는 생성물이
    실마리·초대를 통째로 빠뜨리는 가장 흔한 실패만 잡는다).
    ...
    **길이 검사를 넣지 않는다.** 상한을 두지 않기로 확정됐다(2026-08-24).
    ...
    if not _has_invitation_signal(text):
