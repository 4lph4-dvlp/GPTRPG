...
export const COPY = {
  ...
  /**
   * 오프닝 자동 발동 대기 표시(Phase 13, SCENE-01, D-02) — 오프닝은
   * 사람이 누르는 1차 동작이 없다(명단 잠금 즉시 화면이 스스로 부른다).
   * 「지금 뭔가 되고 있다」를 말해 줄 유일한 한 줄이 이것이다.
   */
  openingWaiting: "진행자가 이야기를 여는 중이에요",
  /**
   * 오프닝 재시도(Phase 13, D-09) — 예외 상황 전용이다. AI가 이상하게
   * 답한 갈래는 서버가 시나리오 원문으로 조용히 대체하므로 화면에
   * 오류가 안 보인다 — 이 단추는 오프닝 API 요청 자체가 실패했을
   * 때만 뜬다(12.3 D-13의 `creationAnnounceRetry`와 같은 모양).
   */
  openingRetry: "오프닝을 다시 불러오기",
  turnFailed: "이번 턴을 처리하지 못했어요. 다시 시도해 주세요",
  /**
   * 서사가 끝내 실패해 이 턴 전체가 자동으로 되돌려졌다는 문구(판 15,
   * D-33/MEAS-02 보완) — 옛 문구("주사위 결과는 그대로예요" + 재시도
   * 단추, 63aef0c)를 대체한다. 그 설계의 전제("다시 시도"가 같은 판정을
   * 다시 서술하는 것)가 틀렸다는 사용자의 정정 이후, 이제는 판정 자체가
   * 취소됐다는 것과 「다시 시도해 주세요」가 곧 「새로 행동을 입력해도
   * 된다」는 뜻임을 함께 말한다. 재시도 단추가 없다 — 시스템은 더 이상
   * 아무것도 하지 않는다.
   */
  turnVoided: "이 턴은 없었던 일이 됐어요. 방금 굴린 주사위도 취소됐어요 — 새로 행동을 입력해 보세요",
  /** 취소된 판정 줄 옆에 붙는 짧은 꼬리표 — `calc-segment__discarded-tag`
   * 스타일을 그대로 재사용한다(새 경고 표면을 만들지 않는다). */
  turnVoidedTag: "취소됨",
  ...
  /**
   * 명부 블록 라벨(Phase 13-06, D-23~D-24, SCENE-05) — `StatusPane` 맨
   * 아래 접힌 줄. **「만난 사람」이 아니라 「지금까지 나온 것」 계열로
   * 짓는다** — 명부가 사람과 사물을 한 그릇에 담으므로(D-17) 사람만
   * 가리키는 이름을 쓰면 사물 항목이 그 아래 있는 것이 어색해지고,
   * 화면만 사람으로 거르고 싶은 유혹이 생긴다(D-24가 명시적으로
   * 금지한 것). TRPG 전문 어휘를 쓰지 않는다(SCENE-02와 같은 규율 —
   * 사장님이 TRPG 비전문가라는 것이 이 저장소의 전제다).
   */
  rosterLabel: "지금까지 나온 것",
  /**
   * 명부 한 줄의 출생 표시 — 시나리오가 미리 적어 둔 쪽(D-17의 1층).
   * 「이 인물이 원래 있던 인물인가 AI가 방금 지어낸 인물인가」를 화면에서
   * 구분하는 것이 이 목록의 존재 이유다(D-24) — 담담한 사실 표시이지
   * 경고 어투가 아니다.
   */
  rosterOriginScenario: "시나리오",
  /**
   * 명부 한 줄의 출생 표시 — 이번 세션에서 즉흥으로 나와 확정된 쪽
   * (D-17의 2층, D-14). 참가자도 「이건 방금 지어낸 거구나」를 알게 되어
   * 몰입이 약간 깨지는 것은 사장님이 보고 고른 대가다(D-24) — 「AI가
   * 지어낸 것」류의 경고 어투가 아니라 담담한 사실 표시로만 적는다.
   */
  rosterOriginEmerged: "이번에 생김",
} as const;
