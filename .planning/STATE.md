...
**Current focus:** Phase 05 — 실험 실행 — 두 번, 1주 간격

> ⚠️ **마일스톤 0은 완료가 아니다.** 설계만 닫혔고 킬 크리테리아 실험은 미실시다. 이 로드맵의 첫 일은 제품 기능이 아니라 실험이며, Phase 6이 게이트다 — 재미(H1)와 원가(H5)가 프로젝트를 멈출 수 있다.
...
**[해소됨 — 승인 2026-08-05] 잠금 결정 D30의 적용 범위를 좁히는 결정 3건.**
`docs/experiment/hypothesis-scoring-rules.md` §7 · PROJECT.md의 결정 표.

- **D-59:** 이번 두 세션으로 H1(재미)의 킬 판정을 내리지 않는다 → 탐색으로 격하. **Phase 6 게이트가 「통과」를 낼 수 없다** — M1 착수 전에 재실험이 들어간다. 로드맵 게이트 표에 「유보」 행을 새로 넣었다
- **D-60:** H2의 판정 근거를 `failure_to_clock_ratio`에서 관찰 기록으로 옮긴다(비율이 항상 3.0에 고정되어 정보량 0). ②③ 구현은 M1-09
- **D-61:** 실험의 목표 범위를 「첫 에피소드(1~2칸)」로 잡는다. 시계 데이터는 4칸 그대로 두고 파국을 목표로 삼지 않는다. 부수 효과로 집계에 `episodes` 칸이 생겨 **무료 체험 원가가 추정에서 실측으로** 바뀌었다 — D21 표의 「1,000팀 $2,500~6,000」의 입력값이 처음으로 존재한다

**남은 판단 항목은 없다.** 세션 진행에 필요한 결정은 전부 닫혔다.

**[문서 정리 — 여전히 미해결] 3건.** 상세는 `.planning/INGEST-CONFLICTS.md`.

1. **원가 수치의 계산 전제가 결정과 어긋난다.** 원가 문서는 「최근 대화 20턴」으로 1인 1시간당 $0.30을 뽑았는데 확정 규칙은 「초기값 10턴」이고, 10턴 기준으로 다시 계산한 문서가 없다. → Phase 5 실측으로 대체하고 Phase 6에서 재계산. **실측 계측 자체는 준비 완료** — 입력·출력·캐시 토큰이 따로 집계되고 계산식이 채점 규칙 §5에 고정되어 있다. 남은 유보 둘: 압축(`context_summarizer`) 미구현 상태의 값이고, 캐시 적중이 0으로 보고될 수 있어 그때는 상한선이다
2. **v1에만 있고 유지·폐기 진술이 없는 구현 약속 2건.** ① 에이전트 응답 껍데기 + 가벼운/중간 모델 타임아웃·재시도 ② 연결 유지 신호 주기와 스트리밍 중 재연결 이어붙이기. → 실험을 돌릴 최소한만 정하고 제품 규격은 M1에서
3. **폐기된 v1 기획서에 폐기 표기가 없다.** 파일명 말고 단서가 없어서 그 파일만 연 사람은 폐기된 설계를 유효한 명세로 읽는다. → 파일 머리에 상태 한 줄을 넣거나 이후 수집에서 제외

**[M1로 넘긴 것 — 새로 드러남]** M1-09(시계 진행 규칙 ②③) · M1-10(폴링 읽기 비용) ·
M1-11(D-11의 3주 재개 검증) · M1-12(`context_summarizer`) · M1-13(D-31의 관계 장부 주입).
근거는 `docs/PIPELINE.md` §9-B.
...
Last session: 2026-08-03T12:09:07.197Z
Stopped at: Phase 5 context gathered
Resume file: .planning/phases/05-1/05-CONTEXT.md

**다음 행동:** Phase 1의 6개 계획이 전부 끝났다. 오케스트레이터가 단계 수준 검증(phase.complete)을 진행한다.
