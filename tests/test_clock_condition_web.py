...
class _ClockJudgeDouble:
    ...
    첫 호출(관문, `judge_clock_signal`)에는 `signal_response`를, 두 번째부터는
    `condition_response`(깊은 판단, `judge_clock_condition`)를 돌려준다.
    `always_raise`면 매번 예외를 던진다(D-05 실패 경로 시험용).
    """
    ...
    def complete(self, *, model, system, messages, max_tokens, timeout_s) -> AgentResult:
        ...
        value = self.signal_response if len(self.calls) == 1 else self.condition_response
